...
class Dataset:
    ...
    def load_learners(self, replace_unk=1):
        """Load learner CV profiles into ``self.learners``.
    ...
    def get_avg_applicable_jobs(self, threshold, indices=None):
        ...
        Args:
            threshold (float): Job matching threshold.
            indices (array-like, optional): Row indices into ``self.learners``.
                When omitted, averages over all learners.

        Returns:
            float: Mean applicable job count.
        """
        if indices is None:
        ...
        total = 0
        for idx in indices:
            total += self.get_nb_applicable_jobs(self.learners[idx], threshold)
        return total / len(indices)

    def get_avg_applicable_jobs_for_profiles(self, profiles, threshold):
        """Average applicable jobs over an array of skill vectors (e.g. post-recommend).

        Args:
            profiles (np.ndarray): Shape ``(n_learners, n_skills)``.
            threshold (float): Job matching threshold.
        ...
        if len(profiles) == 0:
            return 0.0
        total = sum(
            self.get_nb_applicable_jobs(profile, threshold) for profile in profiles
        )
    ...
    def get_avg_learner_attractiveness(self, indices=None):
        """Average job-market attractiveness over learners, optionally by split.

        Args:
            indices (array-like, optional): Row indices into ``self.learners``.

        Returns:
            float: Mean attractiveness score.
        """
        if indices is None:
            indices = np.arange(len(self.learners))
        if len(indices) == 0:
            return 0.0
        total = sum(
            self.get_learner_attractiveness(self.learners[idx]) for idx in indices
        )
        return total / len(indices)

    def get_avg_learner_attractiveness_for_profiles(self, profiles):
        """Average attractiveness over skill vectors (e.g. after recommendations).
        ...
        if len(profiles) == 0:
            return 0.0
        total = sum(self.get_learner_attractiveness(profile) for profile in profiles)
        return total / len(profiles)
