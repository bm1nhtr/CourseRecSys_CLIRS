...
Uses Utils.results_paths layout:
  Results/{lineage}/steps_{steps}/data_{data_seed}/courses_{nb_courses}/k_{k}/
"""
            ...
            for data_dir in sorted(os.listdir(steps_path)):
                ...
                if course_dirs:
                    for courses_dir in course_dirs:
                        courses_path = os.path.join(data_path, courses_dir)
                        k_dirs = [
                            name
                            for name in sorted(os.listdir(courses_path))
                            if os.path.isdir(os.path.join(courses_path, name))
                            and name.startswith("k_")
                        ]
                        if k_dirs:
                            for k_dir in k_dirs:
                                cell = os.path.join(courses_path, k_dir)
                                found = True
                                n_csv, n_raw = _cell_stats(cell)
                                print(
                                    f"- {lineage}/{steps_dir}/{data_dir}/"
                                    f"{courses_dir}/{k_dir}"
                                )
                                print(f"    sweeps: {n_csv} csv | raw: {n_raw} files")
                        elif os.path.isfile(
                            os.path.join(courses_path, "manifest.json")
                        ):
                            # Legacy layout without k_* subfolder
                            found = True
                            n_csv, n_raw = _cell_stats(courses_path)
                            print(
                                f"- {lineage}/{steps_dir}/{data_dir}/"
                                f"{courses_dir} (legacy, no k_/)"
                            )
                            print(f"    sweeps: {n_csv} csv | raw: {n_raw} files")
